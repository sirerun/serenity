...
//
// Sweep, Consolidate, Decay, and SLO are placeholder bodies until their
// owning tasks land (T2.6 expiry sweeper, T2.14 consolidate, T2.12 decay,
// T2.15 queue SLOs — docs/plans/E2-m2-reconcile.md). T2.19's own acceptance
// bar only requires the runner scaffold, the CLI surface, and idempotent
// run-record semantics to be real and tested; the scheduled work itself is
// intentionally not implemented here. Each placeholder only records that
// it ran (recordRun) so `serenity cron <job>` on a fixture exits 0 and a
// second run is a genuine no-op rather than an error. Whoever ships T2.6/
// T2.12/T2.14/T2.15 replaces the matching function body in place — same
// name, same signature, same registry entry — and this package needs no
// other change.
package cron
