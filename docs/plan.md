...
### E4 -- M4: serve + protocols  -> docs/plans/E4-m4-serve-protocols.md  (7/19, gated behind M2+M3; T4.3 and T4.18 were the cross-epic-startable tasks during E3 and both have now shipped -- ADR 011; T4.12 shipped 2026-09-07 (PR #88) and T4.10 shipped 2026-09-07 (PR #119), both cross-epic-startable the moment T2.1 landed, needing no other E2 wave-2a task -- see section 5; T4.18/T4.19 added 2026-09-02, ADR 012; T4.4 shipped 2026-09-07 (PR #128, DISPOSITION v1 server -- `internal/server/disposition`: list_pending/dispose/capture/subscribe over the already-shipped `internal/disposition.Store`/`internal/events.Store`, the events package's first live producer -- see docs/roadmap.md Shipped for full detail), deps [T2.1, T2.20, T4.12, T4.3] all shipped by 2026-09-07. T4.1 (`serenityd` core) shipped 2026-09-07 (PR #140 -- ticker loop embedding `internal/cron`'s job functions per ADR 006, pidfile single-instance guard, graceful shutdown, `log/slog` structured logging -- see docs/plans/E4-m4-serve-protocols.md for the full annotation); T4.2 (MCP stdio transport, deps [T4.1]) is now unblocked. T2.17 shipped 2026-09-07 (briefing scaffold, see E2 above) -- with T3.7 also already shipped, T4.6 shipped 2026-09-07 (PR #TBD, DIRECTION v1 server: brief/check_plan/propose over `internal/server/direction`, mirroring T4.4's own pattern; brief packs RFC 0001 §12's four sections through `internal/briefing.Pack` (T2.17) unchanged; check_plan's wire verdict is produced by the same `check.ToWire` converter `serenity check --json` now also calls (extracted from `internal/cli/check.go` in the same PR), so the two surfaces can never drift; propose never calls a ledger write method, only `disposition.Store.Create` -- see docs/roadmap.md Shipped for full detail), and T4.7 (JSON Schemas + `serenity protocol --json`, deps [T3.7, T2.17]) is now genuinely unblocked)
...
- [x] T4.6 DIRECTION v1 server: brief (token budget governs, caps 12/8/8/5, omitted counts, budget_estimator named), check_plan (schema-primary verdict), propose (lands in the queue, never mutates a precept) (merged PR #TBD, 2026-09-07)
