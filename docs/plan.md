...
### E1 -- M1: ingest spine + honest evals  -> docs/plans/E1-m1-ingest.md  (36/37; T1.23 (David's own Gmail/repo exit verification, RFC M1 AC) completed 2026-09-05/06, remediation from that run minted waves 1g/T1.28-T1.35. T1.29 closed 2026-09-07 (PR #179): all 12 ava extraction families named in its acc line now clear the recall floor, after three full live-eval acceptance runs each surfacing one more genuine gap. T1.35 closed in the same PR (the `prefers` familyGuidance example that leaked a held-out span). Only T1.34 remains open (T1.29 substantively fixed its root cause but left the acc line's own dedicated write-up artifact for a coordinator call). T1.24/T1.25 (wave 1e, OpenRouter provider configurability, ADR 013) shipped 2026-09-05 (PR #63, PR #66), additional, not part of the RFC AC)
