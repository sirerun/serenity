...
### E4 -- M4: serve + protocols  -> docs/plans/E4-m4-serve-protocols.md  (14/21; T4.20 pinned memory repair shipped in PR #160, closing T4.5 shape acceptance and verifying the migrated T4.9 drift/T4.11 security suites. T4.7 schema publication shipped in PR #162, publishing draft-2020-12 schemas for every MEMORY_VERBS/DISPOSITION/DIRECTION wire object plus `serenity protocol --json`. Authenticated HTTP MCP assembly remains T4.21; full conformance remains T4.14. See the epic file and roadmap for shipped transport, Claude setup, facade, spend and protocol-handler details.)
