...
### E1 -- M1: ingest spine + honest evals  -> docs/plans/E1-m1-ingest.md  (37/37, complete; T1.23 (David's own Gmail/repo exit verification, RFC M1 AC) completed 2026-09-05/06, remediation from that run minted waves 1g/T1.28-T1.35. T1.29 closed 2026-09-07 (PR #179): all 12 ava extraction families named in its acc line now clear the recall floor, after three full live-eval acceptance runs each surfacing one more genuine gap. T1.35 closed in the same PR (the `prefers` familyGuidance example that leaked a held-out span). T1.34 closed 2026-09-07 (coordinator call, documentation only): both directions of the said/prefers confusion T1.29 fixed are now named by specific held-out spans in docs/devlog.md. T1.24/T1.25 (wave 1e, OpenRouter provider configurability, ADR 013) shipped 2026-09-05 (PR #63, PR #66), additional, not part of the RFC AC)
...
### E4 -- M4: serve + protocols  -> docs/plans/E4-m4-serve-protocols.md  (21/21; T4.17 external-session exit completed 2026-09-08, evidence in docs/evals/m4-report.md; T4.20 pinned memory repair shipped in PR #160, closing T4.5 shape acceptance and verifying the migrated T4.9 drift/T4.11 security suites. T4.7 schema publication shipped in PR #162, publishing draft-2020-12 schemas for every MEMORY_VERBS/DISPOSITION/DIRECTION wire object plus `serenity protocol --json`. T4.16 protocol documents shipped in PR #164, publishing docs/protocol/MEMORY_VERBS_v1.md/DISPOSITION_v1.md/DIRECTION_v1.md with governance sections and a docs test asserting every schema $id is linked. T4.13 conformance fixture set shipped in PR #165, vendoring gbrain's memory-verbs cases.json plus real HTTP-recorded DISPOSITION and DIRECTION transcripts under testdata/conformance/, each pinned by a sha256 MANIFEST verified on every `go test` run. T4.21 authenticated Streamable HTTP MCP endpoint shipped in PR #166, extracting stdio's dispatch into a transport-agnostic `Session` type shared by a new `HTTPHandler` (`serenity serve --http`) over the existing loopback/bearer/mTLS transport. T4.19 `pkg/serenity` facade `Brief` shipped in PR #168, calling T4.6's `BuildBrief` directly and extending T4.18's drift test with every T4.13 DIRECTION transcript. T4.14 full conformance shipped in PR #169, a new `gbrain-protocol-conformance` CI job running gbrain's own SDK client against a live `serve --http` endpoint on a throwaway fixture brain, in-process to keep the daemon auth token in the package's already-mocked keychain. T4.15 Serenity conformance replay shipped in PR #171, replaying all three pinned protocol fixture families against a live target with dynamic-field normalization and JSON reporting. The M4 exit (T4.17) is complete; E4 has no remaining tasks. See the epic file and roadmap for shipped transport, Claude setup, facade, spend and protocol-handler details.)
### E5 -- M5: migration + launch  -> docs/plans/E5-m5-migration-launch.md  (11/15; T5.8 nightly 10K ingest and real slowdown-rejection evidence completed 2026-09-08; T5.13 public synthetic fixture and pinned CI completed 2026-09-08; T5.3 crash-safe resumable import completed 2026-09-08; T5.2 field-level round-trip audit and JSON report completed 2026-09-08; T5.9 deterministic 10K-message mbox generator completed 2026-09-08, unblocking T5.8; T5.1 gbrain file importer implemented 2026-09-08 with metadata-preserving canonical pages and CLI wiring; T5.11's weekly report card shipped earlier (PR #149); T5.5 adversarial corpus release gate shipped in PR #173, wiring T1.20's existing corpus suite into release.yml as a required job. T5.4 mkdocs-material docs site shipped in PR #175, wiring already-shipped operator/protocol content into a strict-build site with a build-time threat-model include, unblocking T5.7/T5.10. T5.7 fresh-machine install-verify script + CI job shipped in PR #177, proving the documented init->connect->sync->extract->search->ask->inbox->check walkthrough works end to end from a clean clone in <15min. T5.10 BrainBench trend persistence + docs chart shipped in PR #178, completing T1.21's deferred durable-history half with real data on a data-only orphan results branch. M4 exit is complete; T5.1/T5.2/T5.3/T5.9 are complete; T5.8/T5.13 are complete; naming, the real-mailbox run, the name-dependent README, and release remain open -- ADR 011)
...
### E15 -- Paused-write review handoff -> docs/plans/E15-pending-handoff.md (1/1; durable paused-write handoff and interactive inbox recovery verified, 2026-09-08)
### E16 -- Reviewed human edit publication -> docs/plans/E16-dirty-edit-publication.md (1/1; explicit human edit publication and crash recovery verified, 2026-09-08)
...
### E22 -- Final import performance investigation -> docs/plans/E22-import-budget-investigation.md (0/1; two production timing failures retained; paired comparison did not reproduce a code slowdown, 2026-09-08)
...
- [x] T4.21 Authenticated Streamable HTTP `/mcp` and live `serenity serve --http` assembly over repaired memory tools; prerequisite of T4.14, deps: [T4.1, T4.2, T4.3, T4.20] (merged PR #166, 2026-09-07, `0ba3ca2` -- see docs/plans/E4-m4-serve-protocols.md for the full annotation)
- [x] T4.14 Run `gbrain protocol conformance --target http://127.0.0.1:<port>/mcp --token <t>` in CI against a throwaway fixture brain (Bun installed in the job); deps: [T4.5, T4.13, T4.20, T4.21] (merged PR #169, 2026-09-07, `9eb3203` -- see docs/plans/E4-m4-serve-protocols.md for the full annotation)
- [x] T4.15 Serenity conformance command: `serenity protocol conformance --target <url>` replaying testdata/conformance transcripts for all three protocols (merged PR #171, 2026-09-07, `0cc4aaf` -- see docs/plans/E4-m4-serve-protocols.md for the full annotation)
...
- [x] T4.19 `Brief` on the `pkg/serenity` facade over T4.6's packer; drift test extended with T4.13 transcripts (ADR 012) (merged PR #168, 2026-09-07, `c20833a` -- see docs/plans/E4-m4-serve-protocols.md for the full annotation)
...
- [x] T5.1 gbrain importer: page frontmatter, facts fence, takes fence, timeline, links -> entity pages, claims, timeline fences, graph edges (`internal/import/gbrain`)
- [x] T5.2 Field-level round-trip test + unmapped-field report
- [x] T5.3 Resumable import: per-(page,row) checkpoint in .serenity/import/gbrain.json written after the batch is durable
...
- [x] T5.5 Adversarial corpus release gate (`evals/adversarial`, required check on release.yml): injected instructions, contradictory and stale sources, false claims, entity collisions, poisoned documents, precept-fabrication attempts, effect-request forgery
- [x] T5.9 10K-message synthetic corpus generator (seeded, deterministic) for the import budget benchmark
...
- [x] T5.7 Fresh-machine install script + CI job on a clean container with the canned Ava corpus: clone -> init -> connect one connector -> sync -> extract -> search -> ask -> inbox -> check, timed
- [x] T5.8 Import budget benchmark: 10K messages through full ingest on cached model outputs, per-stage timing artifact, nightly regression compare
- [x] T5.10 BrainBench trend published: nightly job appends to evals/brainbench-trend.json on a results branch and renders a chart into the docs site
...
- [x] T5.13 Public fixture gbrain brain: publish testdata/gbrain-fixture as a standalone public repo with README crediting gbrain; CI clones it at a pinned sha for the round-trip test
