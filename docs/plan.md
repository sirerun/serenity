...
### E5 -- M5: migration + launch  -> docs/plans/E5-m5-migration-launch.md  (3/15; T5.11's weekly report card shipped earlier (PR #149); T5.5 adversarial corpus release gate shipped in PR #173, wiring T1.20's existing corpus suite into release.yml as a required job. T5.4 mkdocs-material docs site shipped in PR #175, wiring already-shipped operator/protocol content into a strict-build site with a build-time threat-model include, unblocking T5.7/T5.10. Remaining tasks gated behind M1+M4; T5.1 and T5.9 carry blocked-by: [T4.17] mechanically -- ADR 011)
