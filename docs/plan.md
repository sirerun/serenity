...
### E4 -- M4: serve + protocols  -> docs/plans/E4-m4-serve-protocols.md  (11/21, T4.5 shape acceptance reopened pending T4.20; T4.9 (CLI vs protocol drift tests) shipped 2026-09-07 (PR #157) -- search/recall, ask/synthesize, check/check_plan, inbox dispose/dispose, and brief/brief drift tests over one fixture; asserts CLI<->protocol-handler parity only, a different axis from T4.20's still-open conformance repair, see docs/roadmap.md Shipped for full detail; T4.11 (server security battery) shipped 2026-09-07 (PR #155) -- redaction-before-egress wiring (internal/redact had zero production callers before this task) and an arbitrary-file-write fix in MEMORY_VERBS's remember verb (Subject flowed unsanitized into store path-construction), plus fuzz targets, an adversarial idempotency-replay test, and an HTTP traversal test; its own acc line doesn't reference the pinned gbrain contract T4.20 repairs, unlike T4.5's, so it is checked despite T4.20 not having shipped -- see docs/plans/E4-m4-serve-protocols.md for the full disclosed reasoning; T4.3 and T4.18 were the cross-epic-startable tasks during E3 and both have now shipped -- ADR 011; T4.12 shipped 2026-09-07 (PR #88) and T4.10 shipped 2026-09-07 (PR #119), both cross-epic-startable the moment T2.1 landed, needing no other E2 wave-2a task -- see section 5; T4.18/T4.19 added 2026-09-02, ADR 012; T4.4 shipped 2026-09-07 (PR #128, DISPOSITION v1 server -- `internal/server/disposition`: list_pending/dispose/capture/subscribe over the already-shipped `internal/disposition.Store`/`internal/events.Store`, the events package's first live producer -- see docs/roadmap.md Shipped for full detail), deps [T2.1, T2.20, T4.12, T4.3] all shipped by 2026-09-07. T4.1 (`serenityd` core) shipped 2026-09-07 (PR #140 -- ticker loop embedding `internal/cron`'s job functions per ADR 006, pidfile single-instance guard, graceful shutdown, `log/slog` structured logging -- see docs/plans/E4-m4-serve-protocols.md for the full annotation); T4.2 (MCP stdio transport, deps [T4.1]) shipped 2026-09-07 (PR #143, real `serve --stdio` entry point, immutable tool registry, handshake/errors/cancellation/limits, stderr-only diagnostics -- registry stays empty until T4.5/T4.6 wire tools). T2.17 shipped 2026-09-07 (briefing scaffold, see E2 above) -- with T3.7 also already shipped, T4.6 shipped 2026-09-07 (PR #146, DIRECTION v1 server: brief/check_plan/propose over `internal/server/direction`, mirroring T4.4's own pattern; brief packs RFC 0001 §12's four sections through `internal/briefing.Pack` (T2.17) unchanged; check_plan's wire verdict is produced by the same `check.ToWire` converter `serenity check --json` now also calls (extracted from `internal/cli/check.go` in the same PR), so the two surfaces can never drift; propose never calls a ledger write method, only `disposition.Store.Create` -- see docs/roadmap.md Shipped for full detail); T4.5 implementation merged 2026-09-07 (PR #150, pinned-shape acceptance reopened pending T4.20; MEMORY_VERBS v1 server: recall/remember/entity/synthesize/forget over MCP, wired live into `serve --stdio` -- unlike T4.4/T4.6, which both stayed package-complete but not live-wired; remember is `internal/reconcile`'s first live production wiring -- see docs/roadmap.md Shipped for full detail), T4.8 shipped in PR #152 (`6c9b6d8`, project-local Claude setup and a blocking pre-plan hook; free-text remains unverified); T4.7 schema publication waits for T4.20 pinned-contract compatibility repair)
...
- [ ] T4.5 MEMORY_VERBS v1 server: recall, remember, entity, synthesize, forget with the gbrain envelope (protocol_version, evidence, provenance, budget meta, cost, enumerated errors with suggestion) plus optional confidence, claim_id, superseded_by (merged PR #150, 2026-09-07, merge commit `4dedd15` -- see docs/plans/E4-m4-serve-protocols.md for the full annotation)
...
- [ ] T4.20 MEMORY_VERBS pinned-v1 compatibility repair (uses merged T4.5 implementation; closes T4.5 acceptance and gates T4.7/T4.9/T4.14 -- T4.11 shipped 2026-09-07 (PR #155) independent of this gate: its own acc line never references the pinned gbrain contract, unlike T4.5/T4.7/T4.9/T4.14's, so it carries no `blocked-by: [T4.20]` annotation in the epic file; see docs/plans/E4-m4-serve-protocols.md for the disclosed reasoning)
...
- [x] T4.9 CLI vs protocol drift tests: search/recall, ask/synthesize, check/check_plan, inbox dispose/dispose, brief/brief over one fixture (PR #157, 2026-09-07 -- see docs/plans/E4-m4-serve-protocols.md for the full annotation; asserts CLI<->protocol-handler parity, a different axis from T4.20's still-open protocol<->gbrain conformance repair)
