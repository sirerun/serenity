...
- [x] Inspect product, reference chat, maker promotion, deployment access, coordination access.
...
Deployment findings: Cloudflare MCP lists no sire.run zone; public nameservers are Google Cloud DNS. GitHub Pages is not enabled. Latest release is v0.1.1; current main has additional capabilities, so documentation must distinguish release and source installs. Coordination check-in published; domain affiliation was not verified during that pass.
