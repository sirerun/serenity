...
- 2026 08 27 T0.10 runtime-state allowlist: implemented independently in a second worktree, `kazi-worktrees/p-61c042a78fbb4652-2314d181-df04d8ea` (branch `kazi-partition/p-61c042a78fbb4652-2314d181-df04d8ea`, commit cadcd95, pushed) -- `index.RuntimeTables` = {jobs, disposition_items, disposition_history, spend_ledger, caches}, schema shells in `migrate()`, `ResetAll` refuses to wipe a listed table, 4 new tests all green (`go test -race` clean repo-wide). This collided with the wave-0a kazi lane's own T0.10 attempt already mid-run in `../wt-T0.10` (branch `task/t0-10-runtime-state-allowlist`, PID 3743/3745, `kazi apply prop-t0-10-runtime-state-allowlist`, started 15:39 same day) -- two independent dispatches converging the same task at once. Did not touch `../wt-T0.10` or its branch (live process, hard rule 3). Whoever reconciles wave 0a should pick one T0.10 result and drop the other; this branch's diff is ready to review either way.
