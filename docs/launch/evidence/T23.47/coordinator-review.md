...
## Reconciliation bypass fixed

Independent review identified that a recent missing-ID attempt was still discarded when reconciliation selected an unrelated subscription. TestReconcileRetainsUnknownCheckoutWithUnrelatedSubscription reproduces this after injected provider-success/local-save failure. The regression failed before correction; now unknown identity is rejected before age handling, and subscription presence never alone deletes checkout attempts. The original direct-closure regression remains separately exercised. The older repair fixture now supplies an actual known session older than25h and proves explicit expiration-based cleanup instead of assuming an unknown session is resolved.22 billing tests pass with race detection; lint zero issues.

Before these latest changes, the full repository race run on68beb94 passed75 packages with5 packages without tests; build lease released. That full-suite result is not evidence for the later bypass fix. Automatic checkout recovery and grace timing remain open.

## Follow-up review and tagged verification

Independent correctness fallback confirmed f339f03 closes the reported reconciliation bypass; no additional concrete finding within that scoped correction. Review was read-only, without test reruns. Coordinator then ran `go test -race -count=1 -tags hostedtest -json ./internal/hosted/billing ./internal/hosted/testhooks` on1d8424e (productionf339f03):22 billing plus8 testhook top-level tests pass, zero failed/skipped. The testhook suite builds actual untagged/tagged probes and verifies the production build ignores its control pipe. Shared build lease released. Receipt counts30 distinct tests across these packages; it does not add the repeated untagged billing run again. Automatic checkout resolution and the separately failing grace-order qualification remain open.
