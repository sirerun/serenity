...
| Restore apply / task50 | `Apply(ctx context.Context, req RecoveryApplyRequest) (RecoveryApplyResult,error)`; `Fence(ctx context.Context, generation int64) (FenceReceipt,error)` on `WriterFencer` | One eligible account per call. `RecoveryApplyResult.Consistent(plan, req)` binds the result to the exact approved plan/request; an unfreeze requires a valid journal seal strictly after the plan watermark, matching its generation, plus provider-verified old-instance stop and revocation of all old credentials/sessions. No new SQL migration. | `internal/hosted/backup/backup.go`; `internal/hosted/service/service.go`; `internal/cli/hosted.go` |
...
**Order.** Stop the old instance, revoke its credentials, seal the journal generation, read the journal through the seal, run `ReconcileCustomer` for each account, then unfreeze eligible accounts atomically per account. The new writer starts generation + 1. Sealing after the stop avoids racing a live writer. `RecoveryPlan.Validate` checks the immutable plan's structural fields and account set. `RecoveryApplyResult.Consistent(plan, req)` requires the request to name that exact plan and one account in its eligible set; the result generation must equal the plan generation. An unfreeze also requires `Fence.SufficientFor(plan)`: the seal hash must be a lowercase SHA-256 digest, its generation must match the plan, and its position must follow the plan's journal watermark. The task50 applier remains responsible for loading the approved plan by hash and verifying provider truth. There is no manual `status='active'` SQL.
...
**Executable specification.** `TestFenceReceiptRequiresEveryFact` (each missing fact refused, all reported, malformed/old/wrong-generation seals rejected), `TestRecoveryResultConsistency` (exact plan hash, eligible single-account binding, result generation and refusal consistency), `TestLocalWriterLockCannotFenceAnotherHost`, and the journal seal scenarios above.
