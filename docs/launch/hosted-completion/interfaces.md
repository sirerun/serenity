...
| Restore apply / task50 | `Apply(ctx context.Context, req RecoveryApplyRequest) (RecoveryApplyResult,error)`; `Fence(ctx context.Context, generation int64) (FenceReceipt,error)` on `WriterFencer` | One account per call. The result carries the immutable plan's generation and its fence receipt must match that generation before unfreeze; also requires provider-verified old-instance stop and revocation of all old credentials/sessions. No new SQL migration. | `internal/hosted/backup/backup.go`; `internal/hosted/service/service.go`; `internal/cli/hosted.go` |
...
**Executable specification.** `TestFenceReceiptRequiresEveryFact` (each missing fact refused, all reported, wrong plan generation rejected), `TestRecoveryResultConsistency`, `TestLocalWriterLockCannotFenceAnotherHost`, and the journal seal scenarios above.
