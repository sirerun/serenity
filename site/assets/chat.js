...
function track(name,outcome){window.serenityAdoption?.emit(name,outcome)}
...
async function ask(q,addUser=true){
 if(asking)return;asking=true;track('chat_started');let failure='unknown';input.value='';input.disabled=true;voice.disabled=true;form.setAttribute('aria-busy','true');
 ...
 try{
  if(!endpoint){failure='configuration';throw new Error('The assistant is being configured. Please use the documentation below.')}
  ...
  if(!response.ok){failure=response.status===429?'rate_limit':'unavailable';const messages={429:'The chat has reached its hourly limit. Please try later or browse the documentation.',503:'The assistant is unavailable right now. You can browse the documentation below.',502:'The AI service could not answer just now.'};throw new Error(messages[response.status]||'Your message could not be sent.')}
  failure='invalid_response';const data=await response.json();if(typeof data.answer!=='string'||!data.answer.trim())throw new Error('The assistant returned an empty answer.');
  track('chat_answered',data.mode);pending.replaceChildren(render(data.answer));if(data.mode==='search'){const label=document.createElement('span');label.className='mode-note';label.textContent='Documentation search · a generated answer is unavailable';pending.append(label)}history.push({role:'user',content:q},{role:'assistant',content:data.answer.slice(0,3000)});if(history.length>8)history.splice(0,history.length-8);
 }catch(err){track('chat_failed',err.name==='AbortError'?'timeout':err instanceof TypeError?'network':failure);error(pending,err.name==='AbortError'?'The assistant took too long to respond.':err instanceof TypeError?'The chat could not be reached.':err.message,q)}
 finally{clearTimeout(timeout);pending.classList.remove('pending-dots');input.disabled=false;voice.disabled=false;asking=false;form.removeAttribute('aria-busy');thread.scrollTop=thread.scrollHeight;input.focus({preventScroll:true});input.scrollIntoView({block:'nearest'})}
}
